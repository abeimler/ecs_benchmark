...
class EnttRuntimeBenchmarkSuite final
    : public ecs::benchmarks::base::ExtendedECSBenchmark<"entt (runtime)", EnttRuntimeApplication,
                                                         entities::EntityFactory> {
public:
  EnttRuntimeBenchmarkSuite() = default;

  explicit EnttRuntimeBenchmarkSuite(ecs::benchmarks::base::ESCBenchmarkOptions options)
  ...
  void BM_IterateSingleComponent(benchmark::State& state) {
    ...
    BM_IterateSingleComponentWithPreCreatedViewCustom(
        state,
        [](auto& registry) {
  ...
  void BM_IterateTwoComponents(benchmark::State& state) {
    ...
    BM_IterateTwoComponentsWithPreCreatedViewCustom(
        state,
        [](auto& registry) {
          ::entt::runtime_view view{};
          view.iterate(registry.template storage<ComponentOne>()).iterate(registry.template storage<ComponentTwo>());
          return view;
        },
        [&](auto& registry, auto& view) {
          view.each([&](auto entity) {
  ...
  void BM_IterateThreeComponentsWithMixedEntities(benchmark::State& state) {
    ...
    BM_IterateThreeComponentsWithMixedEntitiesAndPreCreatedViewCustom(
        state,
        [](auto& registry) {
          ::entt::runtime_view view{};
          view.iterate(registry.template storage<ComponentOne>())
